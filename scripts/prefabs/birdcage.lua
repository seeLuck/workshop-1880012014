...
local function immortal_caged_bird_fn(self)
    local cage = self.components.occupier:GetOwner()
    if cage ~= nil and cage:HasTag("cage") then
        self.components.Hunger.current = 1
        self.components.Hunger:DoDelta(0)
        self.components.Hunger.burning = false
    end
end
...
local function OnEmptied(inst, bird)
    ...
    if bird~= nil then
        bird.components.perishable:StartPerishing();
    end
end
...
return Prefab("birdcage", fn, assets, prefabs),
    ...
    Prefab("huaminisign", miniFn, assets)
