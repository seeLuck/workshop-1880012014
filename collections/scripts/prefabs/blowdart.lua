...
local function onunequipN(inst, owner)
    owner.AnimState:ClearOverrideSymbol("swap_object")
    owner.AnimState:Hide("ARM_carry")
    owner.AnimState:Show("ARM_normal")
end
...
local function commonN(anim, tags)
    local inst = CreateEntity()
    ...
    inst:AddComponent("equippable")
    inst.components.equippable:SetOnEquip(onequipN)
    inst.components.equippable:SetOnUnequip(onunequipN)
